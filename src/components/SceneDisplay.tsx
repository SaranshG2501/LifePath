    ...
    <div className="w-full max-w-4xl mx-auto animate-scale-in">
      <Card className="mb-8 shadow-lg border-indigo-500/20 overflow-hidden bg-gradient-to-br from-indigo-900/30 to-purple-900/30 backdrop-blur-md">
        ...
        <CardHeader className={`pb-2 relative ${scene.image ? 'pt-4' : 'pt-6'}`}>
          {!scene.image && (
            <div className="absolute -top-6 left-6 bg-gradient-to-r from-indigo-500 to-purple-500 text-white px-4 py-2 rounded-xl shadow-md">
              {scene.title}
            </div>
          )}
...
const ChoiceButton: React.FC<{
  ...
  const button = (
    <button
      className="w-full text-left px-4 py-3 rounded-lg bg-gradient-to-r from-indigo-900/60 to-purple-900/60 border border-white/10 hover:border-indigo-300/30 hover:bg-indigo-800/30 transition-all duration-300 flex items-center gap-3 animate-fade-in shadow-md hover:shadow-indigo-500/10"
      onClick={onClick}
      style={buttonStyle}
    >
      {isMobile ? (
        <ArrowRight className="text-indigo-300 flex-shrink-0 h-5 w-5 transition-transform" />
      ) : (
        <ChevronRight className="text-indigo-300 flex-shrink-0 h-5 w-5 transition-transform" />
      )}
