...
import { Radio, Users, Clock, Loader2, Wifi, CheckCircle, XCircle, StopCircle, AlertTriangle } from 'lucide-react';
...
const GlobalSessionNotification: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Listen for classroom updates for immediate notification if user is in a classroom
    if (classroomId) {
      unsubscribeClassroom = onClassroomUpdated(classroomId, async (classroom) => {
    ...
    // Listen for all notifications (session started and session ended)
    const unsubscribeNotifications = onNotificationsUpdated(currentUser.uid, (notifications) => {
      // Find session ended notifications first (highest priority)
      const sessionEndedNotification = notifications.find(n => 
        n.type === 'session_ended' && 
        !dismissedSessions.has(n.sessionId)
      );
      
      // Find session started notifications
      const liveSessionNotification = notifications.find(n => 
        n.type === 'live_session_started' && 
        !dismissedSessions.has(n.sessionId)
      );
      
      if (sessionEndedNotification) {
        setNotification(sessionEndedNotification);
      } else if (liveSessionNotification) {
  ...
  const handleSessionEndedAcknowledge = () => {
    if (notification) {
      setDismissedSessions(prev => new Set(prev).add(notification.sessionId));
      setNotification(null);
      
      // If student is currently in game mode, return to individual mode
      setGameMode("individual");
      setClassroomId(null);
      
      toast({
        title: "Session Ended",
        description: "You've been returned to individual mode.",
      });
  ...
  return (
    <Dialog open={true} onOpenChange={() => {}}>
      <DialogContent className={`bg-gradient-to-br ${notification.type === 'session_ended' ? 'from-red-900/95 to-red-800/95 border-red-500/30' : 'from-slate-900/95 to-slate-800/95 border-green-500/30'} text-white max-w-md backdrop-blur-lg`} hideCloseButton>
        <DialogHeader>
