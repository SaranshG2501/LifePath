...
const FeaturesSection = () => {
  ...
  return (
    <section className="mb-12 sm:mb-16 animate-fade-in" style={{ animationDelay: "0.5s" }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
        {features.map((feature, index) => (
          <Card 
            key={feature.title}
            className={`bg-gradient-to-br ${feature.gradient} border-white/20 backdrop-blur-lg shadow-2xl hover:shadow-3xl transition-all duration-500 hover:scale-105 hover:border-white/30 group animate-fade-in`}
            style={{ animationDelay: feature.delay }}
          >
            <CardContent className="p-6 sm:p-8 text-center">
              <div className={`p-4 bg-gradient-to-br ${feature.gradient} rounded-full w-16 h-16 sm:w-20 sm:h-20 flex items-center justify-center mb-6 mx-auto group-hover:scale-110 transition-all duration-300 shadow-lg`}>
                <feature.icon className={`h-8 w-8 sm:h-10 sm:w-10 ${feature.iconColor}`} />
              </div>
              <h3 className="font-bold text-xl sm:text-2xl mb-4 text-white group-hover:text-blue-100 transition-colors">
                {feature.title}
              </h3>
