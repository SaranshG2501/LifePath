...
export function ParticleTextEffect({ words = DEFAULT_WORDS, className = "", width = 1000, height = 500, showOnce = false }: ParticleTextEffectProps) {
  ...
  const animate = () => {
    ...
    // Auto-advance words only if not showOnce mode, or if we haven't shown once yet
    if (!showOnce || !hasShownOnceRef.current) {
      frameCountRef.current++
      if (frameCountRef.current % 240 === 0) {
        wordIndexRef.current = (wordIndexRef.current + 1) % words.length
        nextWord(words[wordIndexRef.current], canvas)
        
        if (showOnce && wordIndexRef.current === words.length - 1) {
